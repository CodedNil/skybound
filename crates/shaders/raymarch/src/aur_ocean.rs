...
use spirv_std::glam::{FloatExt, Mat2, Vec2, Vec3, Vec3Swizzles};
use spirv_std::num_traits::Float;

use crate::utils::{hash12, hash13, mod1, smoothstep, step};

pub const COLOR_A: Vec3 = Vec3::new(0.6, 0.3, 0.8);
pub const COLOR_B: Vec3 = Vec3::new(0.4, 0.1, 0.6);
pub const FLASH_COLOR: Vec3 = Vec3::new(0.6 * 5.0, 0.6 * 5.0, 1.0 * 5.0);
pub const SUN_COLOR: Vec3 = Vec3::new(0.99, 0.97, 0.96);
pub const OCEAN_TOP_HEIGHT: f32 = 0.0;

const TURB_ROTS: [Mat2; 10] = [
    Mat2::from_cols(Vec2::new(0.600, 0.800), Vec2::new(-0.800, 0.600)),
    Mat2::from_cols(Vec2::new(-0.280, 0.960), Vec2::new(-0.960, -0.280)),
    Mat2::from_cols(Vec2::new(-0.843, -0.538), Vec2::new(0.538, -0.843)),
    Mat2::from_cols(Vec2::new(0.422, 0.907), Vec2::new(-0.907, 0.422)),
    Mat2::from_cols(Vec2::new(-0.644, 0.765), Vec2::new(-0.765, -0.644)),
    Mat2::from_cols(Vec2::new(-0.171, -0.985), Vec2::new(0.985, -0.171)),
    Mat2::from_cols(Vec2::new(-0.942, 0.337), Vec2::new(-0.337, -0.942)),
    Mat2::from_cols(Vec2::new(0.773, -0.634), Vec2::new(0.634, 0.773)),
    Mat2::from_cols(Vec2::new(0.196, -0.981), Vec2::new(0.981, 0.196)),
    Mat2::from_cols(Vec2::new(-0.923, -0.384), Vec2::new(0.384, -0.923)),
];
...
fn compute_turbulence(initial_pos: Vec2, time: f32) -> Vec2 {
    ...
    for i in 0..10 {
        let rot = TURB_ROTS[i];
        let phase =
            freq * (pos.x * rot.x_axis.y + pos.y * rot.y_axis.y) + TURB_SPEED * time + i as f32;
        pos += TURB_AMP * rot.x_axis * phase.sin() / freq;
        freq *= TURB_EXP;
    }
    pos
}
...
fn flash_emission(pos: Vec2, time: f32) -> Vec3 {
    ...
    for c in 0..9 {
        ...
        for i in 0..4 {
            ...
            let flicker = 0.8.lerp(1.0, step(0.5, flicker_time.fract()));
...
pub fn sample_ocean(
    ...
    if density >= 1.0 {
        ...
        fbm_value = noise_texture
            ...
            * 2.0
            - 1.0;
        sample.density = fbm_value * fbm_value * density + smoothstep(-50.0, -1000.0, altitude);
    }
    ...
    sample.color = COLOR_A.lerp(COLOR_B, fbm_value);
    ...
    sample.emission +=
        flash_emission(pos.xy(), time) * smoothstep(-100.0, -800.0, altitude) * sample.density;
