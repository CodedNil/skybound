...
const SUN_POSITION = vec3(2.0, 1.0, 2.0);
...
fn density_at_cloud(pos: vec3<f32>, cloud: Cloud, dist: f32, timeOffsetA: vec3<f32>, timeOffsetB: vec3<f32>) -> f32 {
    ...
    if form == 0 { // Cumulus
        ...
        // Bottom cutoff
        let flat_cut = smoothstep(-0.1 * cloud.scale.y, 0.05 * cloud.scale.y, dpos.y);
        shape *= flat_cut;
...
fn raymarch(ro: vec3<f32>, rd: vec3<f32>, tMax: f32, dither: f32) -> vec4<f32> {
    ...
    let phase = HenyeyGreenstein(SCATTERING_ANISO, dot(rd, sun_direction));
    ...
    while t < tMax && sumCol.a < ALPHA_TARGET {
        ...
        while t < nextEvent && t < tMax && sumCol.a < ALPHA_TARGET {
            ...
            let pos = ro + rd * t;
            var density_this_step = 0.0; // Keep a running total of density at this point
            ...
            for (var i = 0u; i < queuedCount; i = i + 1u) {
                if queuedList[i].enterT <= t && t <= queuedList[i].exitT {
                    ...
                    if density > 0.01 {
                        ...
                        let col = (SUN_COLOR * beer + AMBIENT_COLOR * (1.0 - beer)) * density * get_brightness(cloud.data);
...
fn fragment(in: FullscreenVertexOutput) -> @location(0) vec4<f32> {
    ...
    let depth: f32 = textureLoad(depth_texture, vec2<i32>(pix), 0);
